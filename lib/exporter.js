...
function linkedFilePaths(pair) {
  if (!pair.markdownPath) return [];
  const content = fs.readFileSync(pair.markdownPath, 'utf8');
  const names = new Set();
  for (const match of content.matchAll(/(?:^|[(/])files\/([^\s)"'>]+)/gm)) {
    try {
  ...
  return Array.from(names)
    .map(name => {
      const original = path.join(pair.filesDir, path.basename(name));
      if (fs.existsSync(original)) return original;
      const parsed = path.parse(original);
      return path.join(parsed.dir, `${parsed.name}.skipped-download${parsed.ext}`);
    })
    .filter(filePath => fs.existsSync(filePath))
    .map(relativeOutputPath)
    .sort();
}

function writeArtifactManifest(index) {
  ...
  for (const conv of index.values()) {
    ...
    const row = {
      ...
      update_time: conv.update_time ?? conv.create_time ?? null,
      status: reason ? 'pending' : 'complete',
    };
    if (pair) {
      if (pair.jsonPath) row.json_path = relativeOutputPath(pair.jsonPath);
      if (pair.markdownPath) row.markdown_path = relativeOutputPath(pair.markdownPath);
      row.file_paths = linkedFilePaths(pair);
    } else {
      row.file_paths = [];
    }
  ...
  const manifest = { scope: 'active_and_projects', conversations };
  fs.writeFileSync(
    path.join(CONFIG.outputDir, 'artifact-manifest.json'),
...
function buildExportResult(progress, summary, options = {}) {
  ...
  const manifest = writeArtifactManifest(index);
  const pending = manifest.conversations
    .filter(conv => conv.status === 'pending')
    .map(conv => ({ id: conv.conversation_id, reason: conv.reason }));
  ...
  const result = {
    ...
    artifacts_manifest: 'artifact-manifest.json',
