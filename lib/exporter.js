...
function readPrefix(filePath, limit = 65536) {
...
function readExportedUpdateTime(filePath) {
  try {
    const content = readPrefix(filePath);
    if (filePath.endsWith('.json')) {
      const match = content.match(/"update_time"\s*:\s*(?:"([^"]+)"|([0-9.]+))/);
...
function collectExportedArtifactTimes() {
  ...
  const scanJsonDir = (dir) => {
    ...
    for (const f of fs.readdirSync(dir)) {
      ...
      try {
        const content = readPrefix(path.join(dir, f));
        const id = content.match(/"(?:id|conversation_id)"\s*:\s*"([^"]+)"/)?.[1];
        const updateMatch = content.match(/"update_time"\s*:\s*(?:"([^"]+)"|([0-9.]+))/);
        remember(id, 'json', updateMatch ? (updateMatch[1] || updateMatch[2]) : null);
      } catch {}
    }
  };
...
function failedFileEntries(progress) {
  return Object.entries(progress.failedFileIds || {}).map(([id, value]) => ({
    id,
    reason: typeof value === 'string' ? value : String(value?.error || value?.reason || 'download failed'),
  }));
}

function buildExportResult(progress, summary, options = {}) {
  const index = loadIndex();
  const artifactTimes = collectExportedArtifactTimes();
  const pending = Array.from(index.values())
    .filter(conv => conversationNeedsUpdate(conv, artifactTimes))
    .map(conv => ({ id: conv.id, reason: 'required artifact missing or older than index' }));
  ...
  const fileFailures = failedFileEntries(progress);
