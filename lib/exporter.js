...
function markdownMetadata(filePath) {
  // Нужны только id и update_time из frontmatter — файл читается кусками
  // с начала до закрывающего '---', а не целиком.
  ...
  const frontmatter = head.slice(4, end);
  return {
...
function pendingConversations(index) {
  ...
  for (const conv of index.values()) {
    if (conv._archived === true) continue;
