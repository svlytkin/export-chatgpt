...
async function browserFetch(url, options = {}) {
  ...
  try {
    ...
    // If response body was evicted from Chromium's inspector cache (happens on large JSON payloads),
    // extract the text directly from the rendered page DOM.
    const errMsg = String(err?.message || '');
    if (errMsg.includes('evicted from inspector cache')) {
      console.warn('  [CF bypass] Warning: Response body evicted from inspector cache. Recovering via DOM extraction...');
      body = await page.evaluate(() => document.body.innerText);
      console.warn(`  [CF bypass] DOM recovery complete. Recovered length: ${body ? body.length : 0} chars.`);
    } else {
      throw err;
    }
  }
