...
const RESULT_PREFIX = 'EXPORT_CHATGPT_RESULT_V1 ';
...
function fail(kind, message) {
  ...
  emitResult({ outcome: 'failed', failure: { kind, message } });
  process.exitCode = 1;
}
...
async function main() {
  ...
  let { opts, bearerToken, sessionToken } = setup;
  ...
  if (!validFormats.includes(CONFIG.exportFormat)) {
    fail('internal', `Error: --format must be one of: ${validFormats.join(', ')}`);
    return;
  }
  ...
  if (CONFIG.nonInteractive && !bearerToken && !sessionToken) {
    fail('auth', 'Error: --non-interactive requires --bearer or --token (or their env vars).');
    return;
  }
  ...
  if (!bearerToken && !sessionToken) {
    ...
    if (!bearerToken) {
      fail('auth', 'Error: No authentication token provided.');
      return;
    }
  }
  ...
  if (!accessToken && sessionToken) {
    try {
      ...
      const kind = error.cloudflareError ? 'cloudflare' : (error.authError ? 'auth' : 'source');
      fail(kind, `Failed to get access token from session: ${error.message}`);
      return;
    }
  }
  ...
  try {
    const result = await run(accessToken, { userId });
    if (!userId) {
      fail('internal', 'Export completed but user_id could not be determined from the access token.');
      return;
    }
    ...
    if (error.authError) {
      fail(error.cloudflareError ? 'cloudflare' : 'auth', error.message);
      return;
    }
    fail(error.sourceError ? 'source' : 'internal', `Export failed: ${error.message}`);
  }
}
...
module.exports = { RESULT_PREFIX, emitResult, main };
