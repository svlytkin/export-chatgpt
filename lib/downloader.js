...
function runFileErrors(progress) {
  if (!Object.prototype.hasOwnProperty.call(progress, '_runFileErrors')) {
    Object.defineProperty(progress, '_runFileErrors', {
      value: {},
      enumerable: false,
      writable: true,
    });
  }
  return progress._runFileErrors;
}

function recordRunFileError(progress, fileId, reason) {
  runFileErrors(progress)[fileId] = String(reason || 'download failed');
}

function clearRunFileError(progress, fileId) {
  delete runFileErrors(progress)[fileId];
}

function fileArtifactExists(filesDir, fileId) {
  if (!fs.existsSync(filesDir)) return false;
  return fs.readdirSync(filesDir).some(name => (
    name === fileId || name.startsWith(`${fileId}.`)
  ));
}
...
async function downloadConversationFiles(accessToken, conversationData, filesDir, progress, convIndexEntry) {
  ...
  for (const ref of fileRefs) {
    ...
    if (shouldSkipLargeFile(ref.sizeBytes)) {
      ...
      clearRunFileError(progress, ref.fileId);
    ...
    try {
      ...
      if (dlInfo.status !== 'success' || !dlInfo.download_url) {
        ...
        recordRunFileError(progress, ref.fileId, errorCode);
      ...
      if (isFileNotFoundError(error)) {
        ...
        recordRunFileError(progress, ref.fileId, 'file_not_found');
      ...
      if (error.authError) {
        ...
        recordRunFileError(progress, ref.fileId, 'access_denied');
        saveProgress(progress);
      ...
      recordRunFileError(progress, ref.fileId, error.message);
...
async function downloadProjectFiles(accessToken, project, progress) {
  ...
  for (const file of project.files) {
    ...
    if (shouldSkipLargeFile(file.size)) {
      ...
      clearRunFileError(progress, fileId);
    ...
    try {
      ...
      if (dlInfo.status !== 'success' || !dlInfo.download_url) {
        ...
        recordRunFileError(progress, fileId, errorCode);
        continue;
      }
      ...
      if (isFileNotFoundError(error)) {
        ...
        recordRunFileError(progress, fileId, 'file_not_found');
...
async function retryPendingFiles(accessToken, progress) {
  ...
  for (const conv of mainIndex.values()) {
    ...
    for (const ref of conv.files) {
      if (
        !fileArtifactExists(PATHS.filesDir, ref.fileId)
  ...
  if (fs.existsSync(PATHS.projectsDir)) {
    for (const entry of fs.readdirSync(PATHS.projectsDir, { withFileTypes: true })) {
      ...
      for (const conv of convs) {
        ...
        for (const ref of conv.files) {
          if (
            !fileArtifactExists(filesDir, ref.fileId)
            && !progress.failedFileIds[ref.fileId]
            && !progress.skippedFileIds?.[ref.fileId]
            && passesFilter(ref.type)
          ) {
